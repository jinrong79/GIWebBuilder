...
class ChineseChar extends j79obj
{
	...
	*
	*/
	public static function getPY($s, $isfirst = false) {
        return self::getPY_gb2312(self::utf8_to_gb2312($s), $isfirst);
    }//-/
	...
	*/
    ...
    public static function getPY_gb2312($s, $isfirst = false) {
        $res = '';
        $len = strlen($s);
	...
	*
	*/
	public static function getPY_First($chStr){
		preg_match_all("/./us", $chStr, $matches);
		$result='';
		$chrList=current($matches);
		for($i=0; $i<count($chrList);$i++){
		    
			$result.=self::getPY_FirstChar($chrList[$i]);	
	...
	*
	*/
	public static function getPY_FirstChar($s) {
        $ascii = ord($s[0]);
        if($ascii > 0xE0) {
