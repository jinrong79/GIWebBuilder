...
(function($){
	...
	$.fn.citySelect=function(settings){
		...
		var data_saver_region=this.attr('data-saver-region');
		var data_saver_address=this.attr('data-saver-address');
		
		var ui=$(
		'<div class="value-view">'+
        '    <input name="" id="" class="input_region" type="text" placeholder="-请选择地区-" title="点击选择地区" readonly="readonly" value="" />'+        
        '    <input name="" id="" class="input_address"  type="text" placeholder="请输入地址剩余部分,例如:XXX路X号" />'+
        '</div>'+
    	'<div class="selector">'+
        '<ul class="tab" id="tab"></ul>'+
        '<div class="tabboard" id="board1"><ul class="list"></ul><div class="clear"></div></div>'+
        '<div class="tabboard" id="board2"><ul class="list"></ul><div class="clear"></div></div>'+
        '<div class="tabboard" id="board3"><ul class="list"></ul><div class="clear"></div></div>'+
    	'<a  class="bu_close">X</a>'+
		'</div>');
		...
		var val_prov=$.trim($('#'+data_saver_region).val());
		var val_address=$.trim($('#'+data_saver_address).val());
		
		var text_tip="-请选择-";
		
		if(val_prov!=''){
			$(ui).find('.input_region').val(val_prov);
		}
		if(val_address!=''){
			$(ui).find('.input_address').val(val_address);
		}
		
		
		
		
		//showProv:读取json数据,显示省份列表
		var showProv=function(){	
			
			temp_html='';
			$.each(city_json.citylist,function(i,prov){
				temp_html+="<li data-idx='"+i+"' class='item' data-value='"+prov.p+"'>"+prov.p+"</li>";
			});
			prov_obj.empty();
			city_obj.empty();
			dist_obj.empty();
			...
			$(box_obj).find('#board2').hide();
			$(box_obj).find('#board3').hide();
			
			$(box_obj).find('#board1').show();
			
			box_obj.find('#tab #tab-1').remove();
			box_obj.find('#tab #tab-2').remove();
			box_obj.find('#tab #tab-3').remove();
			box_obj.find('#tab').append('<li id="tab-1" class="cur">'+text_tip+'</li>');
					
			prov_obj.find('.item').click(click1);
		};//-----------------/showProv
		
		//showCity:根据已经选择的省份读取json数据,显示其属城市列表
		var showCity=function(){
			
			if(typeof(city_json.citylist[prov_id].c)=="undefined"){
				submit_value();
				return;	
			}
			...
			$.each(city_json.citylist[prov_id].c,function(i,city){
				temp_html+="<li data-idx='"+i+"' class='item' data-value='"+city.n+"'>"+city.n+"</li>";
			});
			city_obj.empty();
			dist_obj.empty();
			...
			$(box_obj).find('#board3').hide();
			$(box_obj).find('#board2').show();
				
			
			box_obj.find('#tab #tab-2').remove();
			box_obj.find('#tab #tab-3').remove();
			box_obj.find('#tab').append('<li id="tab-2" class="cur">'+text_tip+'</li>');
					
			city_obj.find('.item').click(click2);
		};//-----------------/showCity
		
		//showDist:根据已经选择的省份和城市，读取json数据,显示其属地区列表
		var showDist=function(){	
		
			if(typeof(city_json.citylist[prov_id].c[city_id].a)=="undefined"){
				submit_value();
				return;	
			}
			...
			$.each(city_json.citylist[prov_id].c[city_id].a,function(i,dist){
				temp_html+="<li data-idx='"+i+"' class='item' data-value='"+dist.s+"'>"+dist.s+"</li>";
			});
			dist_obj.empty();
			dist_obj.append(temp_html);	
			
			$(box_obj).find('#board3').show();
			
			box_obj.find('#tab #tab-3').remove();
			box_obj.find('#tab').append('<li id="tab-3" class="cur">'+text_tip+'</li>');
					
			dist_obj.find('.item').click(click3);
		...
		var submit_value=function(){
				var region_str='';
				$.each($(box_obj).find('#tab li'),function(i,li){
					if(typeof($(li).attr('data-idx'))!='undefined' && $(li).attr('data-idx')!=''){
						region_str+=$(li).text()+'/';	
					}
				});
				$(wrap_obj).find('.input_region').val(region_str);
				
				$('#'+data_saver_region).val(region_str);			
		...
		var click1=function(){
			...
			prov_id=$(this).attr('data-idx');	
			
			$(box_obj).find('#tab-1').text($(this).attr('data-value'));
			$(box_obj).find('#tab-1').attr('data-idx', prov_id);
			$(box_obj).find('#tab-1').removeClass('cur');
			
			$(box_obj).find('#board1').hide();
		...
		var click2=function(){
			$(this).siblings('.selected').removeClass('selected');
			$(this).addClass('selected');			
			city_id=$(this).attr('data-idx');	
			
			$(box_obj).find('#tab-2').text($(this).attr('data-value'));
			$(box_obj).find('#tab-2').attr('data-idx', prov_id);
			$(box_obj).find('#tab-2').removeClass('cur');
		...
		var click3=function(){
			$(this).siblings('.selected').removeClass('selected');
			$(this).addClass('selected');			
			dist_id=$(this).attr('data-idx');	
			
			$(box_obj).find('#tab-3').text($(this).attr('data-value'));
			$(box_obj).find('#tab-3').attr('data-idx', prov_id);
		...
		var ini=function(){
			...
			prov_obj=$(box_obj).find('#board1 .list');
			city_obj=$(box_obj).find('#board2 .list');
			dist_obj=$(box_obj).find('#board3 .list');
			...
			viewer_obj.find('.input_address').change(function(){					
				$('#'+data_saver_address).val($(this).val());
			});
